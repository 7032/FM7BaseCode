...
| `$C000-$C1FF` | コンソール処理補助 RAM の先頭部 (公式資料) | 本テンプレは未使用 (安全側マージンとして空けてある。 下記補足) |
...
> **`$C000-$C1FF` を空けている理由**: `$C000-$CFFF` はコンソール処理補助 RAM (公式資料) です。 本テンプレは文字表示・カーソルをサブシステムのコンソール機能に頼らず描画を全部自前で行うので、 理屈の上ではこの 4 KB はまるごと使えます (実際 `$C200` 以降は subprog/sprite/背景タイルに再利用しています)。 先頭の `$C000-$C1FF` (512 byte) を未使用にしているのは、 サブシステムが 20ms の NMI で止まらず動き続ける (後述「サブシステムは止まっていない」) なか、 念のため安全側に倒したマージンです (= 公式資料が「ここを必ず空けよ」と定めているわけではなく、 触っても動く公算は高いが確実性を優先した措置)。 一時用途 (debug マーカー等) には再利用できます。
...
#### sprite / 背景タイルデータを差し替えるときの補足

本テンプレの sprite (= 32x32 × 16 枚) と背景タイル (= 64x64) は、 **わかりやすさ優先で大きめ・単純なデータ**を入れています。 とくに背景タイルは 64x64 モノクロ = B plane 512 byte と、 用途のわりに贅沢な使い方です。 差し替え時の勘所:

- **基本**: `assets/` の画像を差し替えて `make` すれば、 変換スクリプトが `_sprite_data[]` / `_bgtile_data[]` を生成し直します (= 後述のビルド節 / [SPRITE.md](SPRITE.md))。 データサイズが変わると配置先 (`$C700` sprite / `$CB00` 背景タイル) の境界が動くので、 sprite 領域が背景タイル base を、 背景タイルがその先の空き/スタックを侵さないか、 `make` 後にサイズを確認すること (= 上の「sprite データ base の配置注意」 と同じ要領)。
- **背景タイル用 RAM を節約する**: 現状は「64x64 を 1 枚」で 512 byte 使いますが、 もっと小さくできます。
  - **小さい繰り返しタイルにする**: 例えば 16x16 のレンガ風パターンなら B plane 16 line × 2 byte = **32 byte**、 8x8 なら **8 byte** で一面の壁/床になります。 ただし `DRAW_BG` は敷き詰め周期を `asm_subprog.s` の `BG_TILE_W` (= 横 byte 数) / `BG_TILE_H` (= 縦 line 数) で持つので、 タイルを小さくするときは **この 2 定数とデータ生成 (`bgtile_to_asm.py` は現状 64x64 前提)** をあわせて調整します (= データだけでなくサブ側コードも少し直す)。
  - **8x8 キャラの組み合わせで「絵」にする**: 1 枚の大タイルではなく、 小さな 8x8 パターンを数種類用意し、 画面の cell ごとに「どれを置くか」 をマップで指定すると、 床・壁・装飾を敷き分けた背景になります (= タイルマップ)。 同じ 8x8 を使い回すので、 パターン集 (数十〜数百 byte) + マップで多彩な背景を表現でき RAM 効率がよくなります。 設計は [TILEMAP.md](TILEMAP.md) 参照。

> まとめ: 一面同じ模様でよいなら**小さい繰り返しタイル** (16x16 / 8x8) で RAM を節約。 場所ごとに違う絵にしたいなら **8x8 タイル + マップ** ([TILEMAP.md](TILEMAP.md))。
...
subprog コード領域 (= `$C300-$C6FF`) には **動作確認マーカー等を書き込まない**。 コード領域に書くと命令の opcode を破壊する (例: `$C320` への書込みは `do_put_cell` の `addb PARAM` 命令を別命令に化けさせる)。 debug マーカーが必要なら subprog 領域外 (= `$C000-$C2FF` の未使用 RAM、 もしくは sub_takeover の chunk buffer `$D3A0+` 等) を使うこと。
