...
├── docs/                    ← ドキュメント一式 (DETAIL/GAMEMAIN/GAMESUB/CMT/… )
...
│   ├── d77_to_t77_chunks.py ← D77 → T77/WAV(FSK) 変換 (`make t77`。 詳細は docs/CMT.md)
│   ├── trampoline.asm       ← テープ多段ロード用トランポリンのソース (参照用)
│   ├── trampoline_*.bin     ← アセンブル済みトランポリン 5 種 (d77_to_t77_chunks.py が使用)
│   ├── D77TOT77WAV.LICENSE.txt ← d77_to_t77_chunks.py の由来部分 (D77TOT77WAV, MIT) のライセンス
...
本テンプレは下記 2 点で FM-7 本来の挙動を前提にしています。 プログラムによってはこの制約を踏まない作りでも動いてしまうことがありますが、 これらを守らないと環境によって動作が変わることがあります。 動作確認は複数の環境で行っておくと安心です。
...
`make` で生成される D77 (= `build/<NAME>.d77`) は 2D / 40 トラック × 2 サイド × 16 セクタ × 256 バイト構成で、 起動に必要なデータは disk 先頭から **side-major** で連続配置されます。 既定 ORG=`$0400` では IPL 1 sector + 本体 BIN 最大 246 sector = 最大 **247 sector** = track 0 〜 track 7 の一部までを使用します (= 本体 BIN は最大 246 sector ≈ 61 KB)。
...
...
sector 1..7  (track 7/side 1)     : 本体 BIN sec 240..245 (最大時)
```
...
- [CMT.md](CMT.md) — テープ (T77 / WAV) ロード対応の仕組み (`make t77`)
