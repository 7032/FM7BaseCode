...
同梱のディスクイメージ (`mousedemo.d77`) を、 本テンプレートの D77 と同じ要領で**ドライブ 0 にセットして電源 ON** するだけで起動します。 ソースから作り直す場合は各デモのディレクトリで `make` を実行します (LWASM に加えて **Node.js** が必要です。 ディスクイメージの組み立てスクリプト `mkd77.mjs` が Node.js 上で動作するため)。

## 4.3 ヘッドレステスト

各デモには、 公開エミュレータ core ([WebM7](https://github.com/7032/WebM7)) 上でデモをブートして画面内容などを自動検証するヘッドレステストが同梱されています。 各デモのディレクトリで `make test` (中身は Node.js スクリプト) で実行します。 WebM7 の場所は環境変数 `WEBM7_DIR` で、 利用者所有の ROM 一式の場所は環境変数 `FM7_ROM_DIR` で、 それぞれ指定します。 **どちらかが見つからない場合、 テストはエラーにならずスキップして正常終了します**。 検証項目の詳細は各デモの README を参照してください。
