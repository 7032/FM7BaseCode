...
マウスカーソルデモには、 公開エミュレータ core ([WebM7](https://github.com/7032/WebM7)) 上でデモをブートして画面内容などを自動検証するヘッドレステストが同梱されています。 デモのディレクトリで `make test` (中身は Node.js スクリプト) で実行します。 WebM7 の場所は環境変数 `WEBM7_DIR` で、 利用者所有の ROM 一式の場所は環境変数 `FM7_ROM_DIR` で、 それぞれ指定します。 **どちらかが見つからない場合、 テストはエラーにならずスキップして正常終了します**。 検証項目の詳細はデモの README を参照してください。
