...
## リポジトリ移管のお知らせ (2026-08)

本リポジトリは個人アカウントから Organization `7032JP` へ移管しました。

- 新 URL: `https://github.com/7032JP/FM7BaseCode`
- 旧 URL へのアクセスや旧 URL で clone 済みのリポジトリは、GitHub のリダイレクトによりそのまま利用できます (`git pull` も可能)。
- ただし将来にわたる確実な利用のため、remote の更新を推奨します:

  ```
  git remote set-url origin https://github.com/7032JP/FM7BaseCode.git
  ```

- Star・Issue・履歴はすべて引き継がれています。
...
本テンプレートとは独立に単体で動く機能デモを [functest/](functest/README.md) に収録しています。 各デモは F-BASIC を使わない 2D ディスクイメージ (`.d77`) として起動し、 実機・エミュレータでそのまま動作します。 本節は概要と導線のみで、 操作・実装の詳細は各デモの README を参照してください。

## 4.1 マウスカーソルデモ ([functest/mouse/](functest/mouse/README.md))

マウスカーソルと画面モード切替ボタンを備えたデモです。 FM-7 / FM77AV / FM77AV40 / FM77AV40EX の全機種で同じディスクから起動します。

- **マウス 2 方式対応**: **バスマウス** と **インテリジェントマウス** (ポート 1 / ポート 2) の両方式を読み取れます。 キーボードの `0` / `1` / `2` で方式を切り替え (マウス操作では変わりません)、 現在の方式は画面に常時表示されます
- **画面モード切替**: 画面上の 4 つのボタンをマウス左クリックすると 640x200 8色 (全機種) / 320x200 4096色 (FM77AV 以降) / 640x400 8色 (AV40系) / 320x200 262144色 (AV40系) を切り替えられます。 機種を実行時に判別し、 非対応モードのボタンは無効表示になります。 切替時もカーソルの画面上の物理位置は維持されます
- **状態表示**: 左右ボタンの押下インジケータや診断用の生読み値も画面に表示します

キー・ボタン割当や実装の詳細表は [functest/mouse/README.md](functest/mouse/README.md) を参照してください。

## 4.2 YM2203 CSM 音声合成デモ ([functest/ym2203csm/](functest/ym2203csm/README.md))

FM 音源 (YM2203 / OPN) の **CSM 音声合成モード**で人の声を鳴らすデモです。 FM77AV / FM77AV40 / FM77AV40EX で動作し、 FM-7 では画面モードを変えずに非対応メッセージを表示して停止します (音は鳴らしません)。

- **サンプリングではない**: 元の波形を 10.1725 ms ごとに分析して求めた**ピッチ (基本周波数) と 4 本のフォルマント (周波数 + 振幅)** だけを持ちます。 1 フレーム 14 バイト、 2.95 秒の音声が 4 KB 弱に収まります
- **CSM の仕組み**: チャネル 3 をタイマ A でキーオン自動繰り返しさせ (= ピッチ)、 4 オペレータ独立周波数をフォルマント、 TL を振幅として使います。 フレーム同期はメイン CPU の周期タイマ (2.0345 ms x 5 = 10.1725 ms)
- **画面**: 320x200 4096色で **64 色の画像を全画面表示**します (アナログパレット方式 + RLE 圧縮。 メイン RAM に置ききれないのでディスクから 1 セクタずつ読みながら逐次展開し、 サブ CPU へ渡して VRAM へ書かせます)。 その上に `PUSH ANY KEY` を 2 倍角で重ね書きし、 キーを押すと案内だけが消えて (隠していた矩形の元画像を書き戻すので下の画像は完全に元どおりです) 音声を再生、 鳴り終わると再び案内が出てループします

データ形式やレジスタ設定の詳細表は [functest/ym2203csm/README.md](functest/ym2203csm/README.md) を参照してください。

## 4.3 PSG 音声再生デモ ([functest/psgvoice/](functest/psgvoice/README.md))

FM-7 の内蔵 PSG (AY-3-8910) **だけ**で音声 WAV を再生するデモです。 FM 音源 (YM2203 / OPN) は使わないため、 FM-7 本体のみで動作します (FM-7 / FM77AV で動作確認済み)。 起動して SPACE キーを押すと、 PSG が音声で約 1.6 秒しゃべります (何度でも繰り返せます)。

- **PSG 音量レジスタを DAC に**: ミキサでトーンとノイズを全て止め、 音量レジスタ (R8/R9/R10) を**サンプルごとに書き換える**ことで、 PSG の音量 D/A 変換器 (対数 16 段) をそのまま 4 ビット DAC として使います。 再生はメイン CPU の 163 サイクル固定ループで約 11kHz (データは 11025Hz で作成) を維持します
- **データ形式**: 44100Hz の WAV を 11025Hz へ間引き、 PSG の対数音量特性に合わせたコンパンディング + 誤差拡散で 4 ビット化し、 2 サンプル/バイトにパックします (1.6 秒 = 8849 バイト。 メイン RAM に全量収まるので再生中はディスクにも割込みにも触りません)
- **テープにも対応**: ディスク起動に加え、 `make t77` で CMT (カセットテープ) ロード用の T77 テープイメージと WAV (FSK 音声) を生成できます。 F-BASIC から `CLEAR ,&H13FF` → `LOADM"CAS0:",,R` の 2 行でロードします (デバイス名は `CAS0:` 必須)

再生ループやデータ形式の詳細表は [functest/psgvoice/README.md](functest/psgvoice/README.md) を参照してください。

## 4.4 起動と再ビルド

同梱のディスクイメージ (`mousedemo.d77` / `ym2203csm.d77` / `psgvoice.d77`) を、 本テンプレートの D77 と同じ要領で**ドライブ 0 にセットして電源 ON** するだけで起動します。 ソースから作り直す場合は各デモのディレクトリで `make` を実行します (LWASM に加えて **Node.js** が必要です。 ディスクイメージの組み立てスクリプト `mkd77.mjs` が Node.js 上で動作するため)。 なお CSM デモの音声パラメータを WAV から作り直す場合のみ、 追加で **Python 3 + NumPy + SciPy** が必要です (生成済みのパラメータを同梱しているため、 通常のビルドには不要です)。 PSG 音声再生デモの `make t77` (テープ用生成物) と音声データの再生成には **Python 3** (標準ライブラリのみ) が必要です。

## 4.5 ヘッドレステスト

マウスカーソルデモには、 公開エミュレータ core ([WebM7](https://github.com/7032JP/WebM7)) 上でデモをブートして画面内容などを自動検証するヘッドレステストが同梱されています。 デモのディレクトリで `make test` (中身は Node.js スクリプト) で実行します。 WebM7 の場所は環境変数 `WEBM7_DIR` で、 利用者所有の ROM 一式の場所は環境変数 `FM7_ROM_DIR` で、 それぞれ指定します。 **どちらかが見つからない場合、 テストはエラーにならずスキップして正常終了します**。 検証項目の詳細はデモの README を参照してください。
