...
同梱のディスクイメージ (`mousedemo.d77` / `ym2203csm.d77`) を、 本テンプレートの D77 と同じ要領で**ドライブ 0 にセットして電源 ON** するだけで起動します。 ソースから作り直す場合は各デモのディレクトリで `make` を実行します (LWASM に加えて **Node.js** が必要です。 ディスクイメージの組み立てスクリプト `mkd77.mjs` が Node.js 上で動作するため)。 なお CSM デモの音声パラメータを WAV から作り直す場合のみ、 追加で **Python 3 + NumPy + SciPy** が必要です (生成済みのパラメータを同梱しているため、 通常のビルドには不要です)。
