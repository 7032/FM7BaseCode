...
本テンプレートとは独立に単体で動く機能デモを [functest/](functest/README.md) に収録しています。 各デモは F-BASIC を使わない 2D ディスクイメージ (`.d77`) として起動し、 実機・エミュレータでそのまま動作します。

現在の収録デモ:

- **[functest/mouse/](functest/mouse/README.md)** — マウスカーソルデモ。 バスマウス / インテリジェントマウスの両方式に対応し、 画面モード切替ボタン (640x200 8色 / 320x200 4096色 / 640x400 8色 / 320x200 262144色) 付き。 FM-7 / FM77AV / FM77AV40 / FM77AV40EX の全機種で同じディスクから起動します

同梱のディスクイメージをドライブ 0 にセットするだけで起動できます。 ソースから作り直す場合は各デモのディレクトリで `make` を実行します (LWASM に加えて **Node.js** が必要です。 ディスクイメージの組み立てスクリプト `mkd77.mjs` が Node.js 上で動作するため)。 詳細は各デモの README を参照してください。
